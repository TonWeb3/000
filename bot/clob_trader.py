...
Prerequisites for live mode (one-time, done OUTSIDE this app):
  - Fund the trading wallet (the address derived from your private key / seed
    phrase) with USDC.e on Polygon.
...
class ClobTrader:
    ...
    def _init_client(self):
        ...
        # EOA only: accept either a hex private key or a 12-word seed phrase, and
        # derive the funder (= the signer's own wallet address) directly from it.
        private_key = resolve_private_key(settings.PRIVATE_KEY)
        ...
        funder = derive_address(settings.PRIVATE_KEY)
        if funder:
